...
Use the host agent's parallel sub-agent mechanism. In Claude, send two `Agent` tool calls with `general-purpose`. In Codex, use the available multi-agent tools. Keep both sub-agents scoped to their axis so their context stays separate.
